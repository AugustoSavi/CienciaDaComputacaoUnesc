...
--  -[X] Você deverá cria uma consulta que emita recibos de venda para todos os pedidos realizados no mês de Abril de 2007 na base Andes.
-- No recibo deverá constar: 
-- -[X] nome do cliente
-- -[X] a data
-- -[X] o valor do pedido 
-- -[X] o número do pedido
-- -[X] o valor por extenso. 

SELECT p.pedidoid AS `Numero pedido`,
	c.nomecliente AS `Nome cliente`,
	DATE_FORMAT(p.data_pedido,'%d/%m/%Y') AS `Data`,
    p.valor_pedido AS `Valor pedido`,
    number_to_words(p.valor_pedido) AS `Valor por extenso`
FROM  pedidos AS p INNER JOIN clientes AS c WHERE c.clienteid = p.clienteid AND MONTH(p.data_pedido) = 4;
